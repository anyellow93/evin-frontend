...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function generarCelda() { return { forma: aleatorio(FORMAS), color: aleatorio(COLORES) }; }

  function svgForma(forma, color, size = 56) {
    const s = size, c = s / 2;
    const filtroId = `sombra_${Math.random().toString(36).slice(2,7)}`;
    const sombra = `<defs><filter id="${filtroId}" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="2" dy="3" stdDeviation="2" flood-color="rgba(0,0,0,0.25)"/></filter></defs>`;
    switch (forma) {
      case 'circulo':
        return `<svg width="${s}" height="${s}" viewBox="0 0 ${s} ${s}" xmlns="http://www.w3.org/2000/svg">${sombra}<circle cx="${c}" cy="${c}" r="${c-5}" fill="${color}" filter="url(#${filtroId})" stroke="rgba(0,0,0,0.15)" stroke-width="2"/></svg>`;
      case 'cuadrado':
        return `<svg width="${s}" height="${s}" viewBox="0 0 ${s} ${s}" xmlns="http://www.w3.org/2000/svg">${sombra}<rect x="5" y="5" width="${s-10}" height="${s-10}" rx="6" fill="${color}" filter="url(#${filtroId})" stroke="rgba(0,0,0,0.15)" stroke-width="2"/></svg>`;
      case 'triangulo':
        return `<svg width="${s}" height="${s}" viewBox="0 0 ${s} ${s}" xmlns="http://www.w3.org/2000/svg">${sombra}<polygon points="${c},5 ${s-5},${s-5} 5,${s-5}" fill="${color}" filter="url(#${filtroId})" stroke="rgba(0,0,0,0.15)" stroke-width="2"/></svg>`;
      case 'rombo':
        return `<svg width="${s}" height="${s}" viewBox="0 0 ${s} ${s}" xmlns="http://www.w3.org/2000/svg">${sombra}<polygon points="${c},5 ${s-5},${c} ${c},${s-5} 5,${c}" fill="${color}" filter="url(#${filtroId})" stroke="rgba(0,0,0,0.15)" stroke-width="2"/></svg>`;
      case 'estrella':
        const puntos = [];
        for (let i = 0; i < 10; i++) {
        ...
        return `<svg width="${s}" height="${s}" viewBox="0 0 ${s} ${s}" xmlns="http://www.w3.org/2000/svg">${sombra}<polygon points="${puntos.join(' ')}" fill="${color}" filter="url(#${filtroId})" stroke="rgba(0,0,0,0.15)" stroke-width="2"/></svg>`;
    }
  }
  ...
  function generarTableros() {
    ...
    for (let i = 0; i < total; i++) tableroBase.push(generarCelda());
    tableroMod = tableroBase.map(c => ({ ...c }));
    const indices = [...Array(total).keys()].sort(() => Math.random() - 0.5).slice(0, numDiferencias);
    indices.forEach(idx => {
      diferencias.add(idx);
  ...
  function renderTablero(panel, tablero, esModificado) {
    ...
    tablero.forEach((celda, idx) => {
      ...
      if (esModificado) div.addEventListener('click', () => manejarClick(idx, div));
  ...
  function manejarClick(idx, celda) {
    ...
    if (diferencias.has(idx)) {
      ...
      if (celdaIzq) { celdaIzq.classList.add('dif-correcta'); celdaIzq.insertAdjacentHTML('beforeend', iconoCorrecto); }

      actualizarContador();
      if (encontradas.size === diferencias.size) finJuego(true);
      ...
      setTimeout(() => { celda.classList.remove('dif-error'); icono.remove(); }, 600);
    }
  }
  ...
  function iniciarTimer() {
    ...
    clearInterval(timerID);
    timerID = setInterval(() => { segundos++; if (tiempoEl) tiempoEl.textContent = segundos; }, 1000);
  }
